...
export default (_client, _config, _jira) => {
    ...
    return [
        new SlashCommandBuilder()
            .setName("upcoming")
            .setDescription(
                "Shows bugs that are likely fixed in the next snapshot"
            )
            .addStringOption((option) =>
                option
                    .setName("project")
                    .setDescription(
                        'The project to search in, for example "MC"'
                    )
            ),

        new SlashCommandBuilder()
            .setName("mcstatus")
            .setDescription("Checks Mojang server status"),
...
async function onMessage(msg) {
    ...
    const regexPattern = new RegExp(
        escapedPrefix + "(" + PROJECTS.join("|") + ")-[0-9]{1,7}",
        "gi"
    );
    ...
    if (piks)
        matches = piks.map((prefixedIssueKey) =>
            prefixedIssueKey.slice(config.prefix.length)
        );
...
async function respondWithIssues(msg, issuesKeys) {
    ...
    if (issuesKeys.length > maxIssuesBeforeBatching) {
        replyNoMention(msg, {
            ...
            const search = `issueKey in (${[...issuesKeys].join(
                ", "
            ...
            await jira
                ...
                .catch(async (error) => {
                    if (
                        ...
                        error.error.errorMessages.includes(
                            "Issue Does Not Exist"
                        )
                    ) {
                        await replyNoMention(
                            msg,
                            `No issue was found for ${issueKey}.`
                        );
                    } else {
                        try {
                            await replyNoMention(
...
async function sendHelp(interaction) {
    await replyNoMention(interaction, {
        embeds: [
            {
                title: config.name + " help",
                description:
                    "I listen for Minecraft bug report links or " +
                    config.prefix +
                    "PROJECT-NUMBER\n" +
                    "For example, saying https://bugs.mojang.com/browse/MC-81098 or " +
                    config.prefix +
                    "MC-81098 will give quick info on those bugs",
                fields: [
                    {
                        name: "Other commands: ",
                        value:
                            ...
                            "**" +
                            config.prefix +
                            "upcoming:** Shows bugs that are likely fixed in the next snapshot.",
                    },
                ],
                url: config.url,
                color: 9441545,
...
function getPageToRender(message) {
    ...
    const description = rederableIssues
        .map(
            (issue) =>
                `\u300B[${issue.key}](https://bugs.mojang.com/browse/${issue.key}) - ${issue.fields.summary}`
        )
        .join("\n");
    ...
    const embed = {
        ...
        color: 0x7ed6df,
        footer: {
            text: `Page ${paginator.currentPage} of ${Math.ceil(
                paginator.issues.length / ITEMS_PER_PAGE
            )}`,
        },
    };
    ...
    const actionRow = new MessageActionRow().addComponents([
        ...
        },
        {
            ...
            disabled:
                ...
                Math.ceil(paginator.issues.length / ITEMS_PER_PAGE),
        },
        {
            type: "BUTTON",
            customId: "done",
    ...
    replyNoMention(interaction, {
        ...
        jira.searchJira(search)
            ...
            .catch((error) => {
                editNoMention(message, {
                    embeds: [
                        { title: "An error has occurred.", color: 0xff0000 },
                    ],
                });
                console.log("Error when processing upcoming command:");
                console.log(error);
...
async function handleButtonClick(interaction) {
    ...
    switch (interaction.customId) {
        case "previous":
            ...
            break;
        case "next":
            paginator.currentPage = Math.min(
                totalPages,
...
async function sendStatus(interaction) {
    ...
    try {
        ...
        for (const service of statuses) {
            ...
            embed.fields.push({
                name,
                value: `:${service[name]}_square: ${service[name]}`,
                inline: true,
            });
        ...
        try {
            await replyNoMention(
                interaction,
                "Could not get status from Mojang API"
            );
        } catch (e2) {
            console.error(e2);
        }
...
async function sendEmbed(interaction, issue) {
    ...
    if (issue.fields.resolution) {
        ...
        if (
            ["Invalid", "Duplicate", "Incomplete", "Cannot Reproduce"].includes(
                resolution
            )
        ) {
