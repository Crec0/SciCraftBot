...
function checkCleanup(msg: Message) {
...
async function checkStreams(gracePeriod: number) {
    const users = [...new Set([...streamMessages].map((m: any) => m.twitchUser))];
    ...
    const online = streams
        ...
        .map((s: any) => s.user_name.toLowerCase());
...
async function getTwitchApi(path: string, params: any) {
...
export default (_client: Client, _config: Config) => {
    client = _client;
    config = _config;
