...
const activePaginators: Map<string, Paginator> = new Map();

export default (_client: Client, _config: Config, _jira: JiraApi) => {
    client = _client;
    config = _config;
    jira = _jira;
...
async function onMessage(msg: Message) {
    ...
    const urlRegex = new RegExp(
        `https?://bugs.mojang.com/browse/(${PROJECTS.join("|")})-[0-9]{1,7}`,
        "gi"
    );
...
async function respondWithIssues(msg: Message | CommandInteraction, issueKeys: string[]) {
...
async function sendHelp(interaction: Message) {
    const embed = new Embed({
        title: `${config["name"]} help`,
        description:
            `I listen for Minecraft bug report links or ${config["prefix"]}PROJECT-NUMBER\n` +
            `For example, saying https://bugs.mojang.com/browse/MC-81098 or ${config["prefix"]}MC-81098 will give quick info on those bugs`,
        fields: [
            {
                name: "Other commands: ",
                value:
                    `**${config["prefix"]}help:** Shows this help screen.\n` +
...
function getPageToRender(message: Message) {
    // get paginator from global cache
    const paginator = activePaginators.get(message.id) as Paginator;
...
async function sendUpcoming(interaction: Message | CommandInteraction, _project: string) {
...
async function handleButtonClick(interaction: ButtonInteraction) {
    const paginator = activePaginators.get(interaction.message.id) as Paginator;
...
async function sendEmbed(interaction: Message | CommandInteraction, issue: IssueObject) {
    ...
    if (issue.fields.customfield_11901) {
        categories = issue.fields.customfield_11901
            .map((category: NameValuePair) => category.value)
            .join(", ");
    }
