...
export default (_client, _config) => {
    ...
    if (config["cleanup-streams"]) {
        ...
        if (
            cleanupConf.twitchApiClientId &&
            cleanupConf.twitchApiClientSecret
        ) {
    ...
    if (config["links-only"]) {
        const linkOnlyConf = config["links-only"];
        const ignoreRoles = linkOnlyConf["ignore-roles"] || [];
        const ignorePermissions = linkOnlyConf["ignore-permissions"] || [
            "MANAGE_CHANNELS",
        ];
        ...
        client.on("messageCreate", (msg) => {
            ...
            for (const [id, role] of msg.member.roles.cache) {
                if (ignoreRoles.includes(id)) {
                    console.log(
                        `${msg.id}: ${msg.author.username} has ${role.name}, not deleting`
                    );
                    return;
                }
            }
            for (const perm of ignorePermissions) {
                if (msg.member.permissions.has(perm)) {
                    console.log(
                        `${msg.id}: ${msg.author.username} has ${perm}, not deleting`
                    );
                    return;
                }
            }
    ...
    if (config["media-only"]) {
        ...
        const ignorePermissions = mediaOnlyConf["ignore-permissions"] || [
            "MANAGE_CHANNELS",
        ];
        ...
        client.on("messageCreate", (msg) => {
            ...
            let img = msg.attachments.values().next().value.name.toLowerCase();
            console.log(img);
...
function checkCleanup(msg) {
    ...
    console.log(
        `Picked up message ${msg.id} linking to ${match[0]} (user ${match[2]})`
    );
    streamMessages.add({
        message: msg,
        twitchUser: match[2],
